...
use frame_support::{
    ...
    dispatch::{DispatchResult, DispatchResultWithPostInfo, IsSubType},
    ensure,
...
use sp_runtime::{
    ...
    PerThing, PerU16, Perbill, RuntimeDebug,
};
#[cfg(feature = "std")]
...
impl<T: Trait> Module<T> {
    ...
    fn do_payout_nominator(
        ...
        let mut nominator_ledger =
            <Ledger<T>>::get(&ctrl).ok_or_else(|| Error::<T>::NotController)?;

        ensure!(
            Self::era_election_status().is_closed()
                || Self::payee(&nominator_ledger.stash) != RewardDestination::Staked,
    ...
    fn do_payout_validator(ctrl: T::AccountId, era: EraIndex) -> DispatchResult {
        ...
        let mut ledger = <Ledger<T>>::get(&ctrl).ok_or_else(|| Error::<T>::NotController)?;
            ...
            Self::era_election_status().is_closed()
                || Self::payee(&ledger.stash) != RewardDestination::Staked,
            Error::<T>::CallNotAllowed,
        );
    ...
    pub fn check_and_replace_solution(
        ...
        // candidates.
        let snapshot_length =
            <SnapshotValidators<T>>::decode_len().ok_or_else(|| Error::<T>::SnapshotUnavailable)?;
